...
def _handle_validate(session: nox.Session) -> None:
    ...
    # Require this PR's own fragment (filename prefix) to match --pr-number.
    # Other changed fragments (e.g. imported from fides on monorepo merge) keep
    # their original pr: values and are only checked for structure/labels.
    if expected_pr_number is not None and validated:
        own_entries: list[tuple[Path, ChangelogEntry]] = []
        mismatched: list[tuple[Path, ChangelogEntry]] = []
        for path, entry in validated:
            match = FILENAME_PATTERN.match(path.name)
            if not match or int(match.group(1)) != expected_pr_number:
                continue
            own_entries.append((path, entry))
            if entry.pr != expected_pr_number:
                mismatched.append((path, entry))

        if not own_entries:
            session.error(
                f"No changelog fragment for PR #{expected_pr_number} "
                f"(expected changelog/{expected_pr_number}-*.yaml in the validated files)"
            )
